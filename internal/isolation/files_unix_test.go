...
func TestViewsStripGroupWorldModesAndRejectSpecialFiles(t *testing.T) {
	...
	for _, selection := range [][]string{{"src/pipe"}, {"src"}, {"src/data", "src/pipe"}} {
		if _, err := views.Create(context.Background(), a.Workspace{Directory: source, Access: a.ReadOnly}, selection); err == nil {
			t.Fatalf("special file accepted through %v", selection)
		}
