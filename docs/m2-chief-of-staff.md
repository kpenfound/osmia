...
`set_status`, `prioritise`, `decide_amendment`, `decide_charter`, `answer`, `escalate`, `relay_ruling`,
...
`file_read`, `file_write`, `ask`, `amend` and `done`. So that its fake workers can ask without a unit, the test grants the mason and
